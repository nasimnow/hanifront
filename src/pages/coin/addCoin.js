...
const AddCoin = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const stocksResponse = await axios.get("https://cors-anywhere.herokuapp.com/https://api.wazirx.com/api/v2/tickers");
      const coinsArray = Object.getOwnPropertyNames(stocksResponse.data)
      const coinsModified = coinsArray.map(coin=>({value:coin,label:coin}))
  ...
  return (
    ...
    <Header/>
      ...
      <div className={styles.title_group}>
        <div onClick={()=>history.push("/add")} className={`${styles.title_main}`}>Transaction</div>
